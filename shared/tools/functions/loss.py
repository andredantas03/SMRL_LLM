...
def perplexity_from_loss(loss: Tensor) -> Tensor:
    """PPL = exp(cross-entropy); clamp evita overflow em losses muito altos."""
    return torch.exp(loss.detach().clamp(max=20.0))